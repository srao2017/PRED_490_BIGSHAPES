...
# This procedure has a list, but it is an empty list; welcome().
    ...
    os.chdir("C:/EDUCATION/NWU/490 - Deep Learning/Week 6")
    #random.seed(1) # set to ensure same set of random numbers across separate runs
    welcome()
    
    mode = input("Train [0] or Run CNN [1] or Run Trained NN Only [2]?[0/1/2]:")
    if mode > 0:
...
# Parameter definitions, to be replaced with user inputs
    ...
    # ALL option means 26 output classes - reads <filename>_classes_26.csv, otherwise GROUPS (default) means 
    # read the <filename>_classes.csv file to get groups
    if mode > 0:
        trainingData, numTrainingDataSets, outputNodes = readTrainingData(filename, "ALL")
    else:        
        trainingData, numTrainingDataSets, outputNodes = readTrainingData(filename)
...
####################################################################################################                
    ...
    # Add the 9 element gray box (GB1) big shape class represented by a 9 element array
    # this will be obtained from the GB1 output which we read in from the previously trained
    # GB1 NN - this now has 92 elements
    if mode > 0: #92 (9+81+2)
